import { signIn } from "@/lib/auth";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";

export default function LoginPage({
  searchParams,
}: {
  searchParams: { callbackUrl?: string; error?: string };
}) {
  return (
    <main className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        {/* Logo */}
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-foreground tracking-tight">Histórias</h1>
          <p className="mt-2 text-muted-foreground text-lg">O livro vivo da sua família.</p>
        </div>

        {/* Erro */}
        {searchParams.error && (
          <div className="mb-6 p-4 border border-destructive bg-destructive/5 text-destructive text-base">
            Erro ao entrar. Por favor, tente novamente.
          </div>
        )}

        <div className="border border-border p-8 bg-card space-y-8">
          {/* Google */}
          <form
            action={async () => {
              "use server";
              await signIn("google", {
                redirectTo: searchParams.callbackUrl ?? "/dashboard",
              });
            }}
          >
            <Button
              type="submit"
              className="w-full h-[72px] text-lg font-bold bg-foreground text-background hover:bg-foreground/90 flex items-center gap-3"
            >
              <svg className="w-6 h-6" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              Entrar com Google
            </Button>
          </form>

          <div className="flex items-center gap-4">
            <div className="flex-1 border-t border-border" />
            <span className="text-muted-foreground text-base">ou</span>
            <div className="flex-1 border-t border-border" />
          </div>

          {/* Magic Link */}
          <form
            action={async (formData: FormData) => {
              "use server";
              const email = formData.get("email") as string;
              await signIn("resend", {
                email,
                redirectTo: searchParams.callbackUrl ?? "/dashboard",
              });
            }}
            className="space-y-4"
          >
            <div className="space-y-2">
              <Label htmlFor="email" className="text-base font-medium">
                Seu e-mail
              </Label>
              <Input
                id="email"
                name="email"
                type="email"
                placeholder="seu@email.com"
                required
                className="h-[56px] text-lg border-border"
                autoComplete="email"
              />
            </div>
            <Button
              type="submit"
              variant="outline"
              className="w-full h-[72px] text-lg font-bold border-2 border-foreground text-foreground hover:bg-secondary"
            >
              Receber link de acesso por e-mail
            </Button>
            <p className="text-sm text-muted-foreground text-center">
              Sem senha. Vocà receberá um link direto no seu e-mail.
            </p>
          </form>
        </div>

        <p className="mt-6 text-center text-sm text-muted-foreground">
          Ao entrar, vocà concorda com nossa{" "}
          <a href="/privacidade" className="underline hover:text-foreground">
            Política de Privacidade
          </a>
          .
        </p>
      </div>
    </main>
  );
}
